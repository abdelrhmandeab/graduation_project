...
# STT
STT_BACKEND = _env("JARVIS_STT_BACKEND", "faster_whisper")  # faster_whisper | egyptalk_transformers
STT_EGYPTALK_ENABLED = _env_bool(
    "JARVIS_STT_EGYPTALK_ENABLED",
    _env_bool("JARVIS_STT_NEMO_EGYPTALK_ENABLED", True),
)
STT_EGYPTALK_MODEL = _env(
    "JARVIS_STT_EGYPTALK_MODEL",
    _env("JARVIS_STT_NEMO_EGYPTALK_MODEL", "NAMAA-Space/EgypTalk-ASR-v2"),
)
STT_EGYPTALK_TRUST_REMOTE_CODE = _env_bool("JARVIS_STT_EGYPTALK_TRUST_REMOTE_CODE", False)
STT_EGYPTALK_CHUNK_SECONDS = max(0.0, _env_float("JARVIS_STT_EGYPTALK_CHUNK_SECONDS", 18.0))
STT_EGYPTALK_STRIDE_SECONDS = max(0.0, _env_float("JARVIS_STT_EGYPTALK_STRIDE_SECONDS", 4.0))
# Egyptian dialect normalization is always enabled for Arabic transcripts.
STT_ARABIC_POST_NORMALIZATION = True
# Keep bilingual behavior for mixed Arabic/English utterances.
STT_EGYPTIAN_DIALECT_ONLY = False
WHISPER_MODEL = _env("JARVIS_WHISPER_MODEL", "small")  # small is the stable CPU default on this hardware.
WHISPER_DEVICE = _env("JARVIS_WHISPER_DEVICE", "cpu")
WHISPER_COMPUTE_TYPE = "int8"
WHISPER_LANGUAGE = str(_env("JARVIS_WHISPER_LANGUAGE", _env("JARVIS_WHISPER_LANGUAGE_HINT", "auto"))).strip().lower()
if WHISPER_LANGUAGE in {"arabic", "ar"}:
    WHISPER_LANGUAGE = "ar"
elif WHISPER_LANGUAGE in {"english", "en"}:
    WHISPER_LANGUAGE = "en"
else:
