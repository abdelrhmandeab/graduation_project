...
export interface StrandsProps {
  ...
  dispersion?: number;
  glassSize?: number;
  /** When false (e.g. reduced motion) the animation loop is paused after one frame. */
  animate?: boolean;
  className?: string;
  style?: CSSProperties;
  /** Static fallback rendered if a WebGL context cannot be created. */
  fallback?: React.ReactNode;
}

type StrandsConfig = Required<
  Omit<StrandsProps, 'className' | 'style' | 'fallback'>
>;
...
export default function Strands({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // No WebGL context available (GPU-less VM, blocked, etc.) — show fallback.
      setFailed(true);
      return;
    }
    ...
    const update = (t: number) => {
      // Pause the loop when reduced-motion is requested or the tab is hidden.
      if (!propsRef.current.animate || !running) {
        return;
      }
    ...
    const onVisibility = () => {
      ...
      if (running && propsRef.current.animate) {
        animateId = requestAnimationFrame(update);
      }
    };
